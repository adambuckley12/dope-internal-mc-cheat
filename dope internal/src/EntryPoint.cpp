...
DWORD __stdcall Main(HMODULE hModule)
{
	...
	if (g_Instance->GetInitializationState() == ERR) {
		MessageBoxA(NULL, "An error occured while initializing client.", "", MB_OK | MB_ICONERROR);
#if DOPE_DEBUG
    ...
    {
		MessageBoxA(NULL, "The client initializing process ended with the INCOMPLETE status, we highly recommend restarting your game to prevent any issues.", "", MB_OK | MB_ICONINFORMATION);
    }

	const auto code = g_Instance->CleanupJVMTI();
	if (code == ERR) {
		MessageBoxA(NULL, "The client cleaning process ended with the ERR status, you may not be safe in screenshare.\nIf this issue still happen please report it and make sure to give the game version and client you are using.", "", MB_OK | MB_ICONINFORMATION);
	}
