	...
	if (Minecraft::GetTheWorld(env) && !Minecraft::GetCurrentScreen(env))
	{
		...
		for (const auto& bb : hitboxToDraw)
		{
			if (espSettings->Visuals.ESP.m_Mode == 1) {
				glColor4f(espSettings->Visuals.ESP.m_FilledColor[0], espSettings->Visuals.ESP.m_FilledColor[1], espSettings->Visuals.ESP.m_FilledColor[2], espSettings->Visuals.ESP.m_FilledOpacity + 0.01f);
				drawFilledBox(bb);

				glColor4f(espSettings->Visuals.ESP.m_OutlineColor[0], espSettings->Visuals.ESP.m_OutlineColor[1], espSettings->Visuals.ESP.m_OutlineColor[2], espSettings->Visuals.ESP.m_OutlineOpacity + 0.01f);
				drawOutlinedBox(bb);
			}
		}
...
void ESP::OnImGuiRender(JNIEnv* env)
{
	...
	const auto DrawBox = [espSettings](const ImVec2& start, const ImVec2& end, const ImU32& color)
	{
		ImGui::GetBackgroundDrawList()->AddRect(start, end, color, 0, 0, espSettings->Visuals.ESP.m_OutlineThickness + 0.01f);
	};
		...
		for (const auto& et : coordsToDraw)
		{
			...
			if (screenBoundingBox.x > 0 || screenBoundingBox.y > 0 || screenBoundingBox.z <= ImGui::GetIO().DisplaySize.x || screenBoundingBox.w <= ImGui::GetIO().DisplaySize.y)
			{
				if (espSettings->Visuals.ESP.m_Enabled && espSettings->Visuals.ESP.m_Mode == 0 && espSettings->Visuals.ESP.m_Boxes) {
					...
					DrawFilledBox(ImVec2(screenBoundingBox.x, screenBoundingBox.y), ImVec2(abs(screenBoundingBox.z - screenBoundingBox.x), abs(screenBoundingBox.w - screenBoundingBox.y)), ImVec4(espSettings->Visuals.ESP.m_FilledColor[0], espSettings->Visuals.ESP.m_FilledColor[1], espSettings->Visuals.ESP.m_FilledColor[2], espSettings->Visuals.ESP.m_FilledOpacity + 0.01f));
