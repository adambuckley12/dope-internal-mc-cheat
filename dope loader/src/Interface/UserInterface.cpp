...
#include "../internalDll/includedDll.h"
#include <Windows.h>
...
/*	
	...
	D3DX11CreateTextureFromMemory(g_pd3dDevice, imgs::rawData, sizeof(imgs::rawData), &info,
		pump, &Image, 0);
	...
	std::vector<std::string> playerEspItems{ "2D", "3D" };
	...
	while (!Instance<Settings>::Get()->m_Destruct)
	{
...
#ifndef _DEBUG
				for (auto timer : Instance<Globals>::Get()->Flags.m_Heartbeats) {
		...
		ImGui::SetNextWindowPos({ 0.F, 0.f });
		{
			...
			if (!bLoaded || !bLogged) {
				...
				ImGui::Text("Tasty Client: Loading... %i%%", iLoadingPercentage);
				ImGui::PopFont();

				if (animStage == 4) {
					std::thread([&] {

						globals->User.userName = "idk how to do the sucky thing";
...
//INJECTION AND NULL OF DLL

							const auto pid = Instance<Globals>::Get()->m_Pid;
							void* buf = malloc(sizeof(tasty_internal_dll));
							std::memcpy(buf, tasty_internal_dll, sizeof(tasty_internal_dll));

							Instance<InjectionHelper>::Get()->Inject(Instance<Globals>::Get()->m_Pid, buf, sizeof(tasty_internal_dll));

							ZeroMemory(buf, sizeof(tasty_internal_dll));
							//ZeroMemory(tasty_internal_dll, sizeof(tasty_internal_dll));
					...
					ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
				{
					...
					ImGui::BeginChild("##modulesChild", { 160.f, m_Height - 150.f }, false);
					{
						...
						ImGui::BeginChild("##modulesBg", { 140.f, m_Height - 150.f });
						{
							...
							const auto versionWidth = ImGui::CalcTextSize("2.0.0");
					...
					ImGui::BeginChild("##moduleChild", { m_Width - 200.f , m_Height - 100.f }, false);
					{
						...
						case 1:
							...
							{
							case 0:
								CreateModuleTab("Player ESP", "Draws a box around each player!", &Instance<Settings>::Get()->Visuals.ESP.m_Bind);
								ImGui::Dummy({ 0.f, 5.f });
								...
								ImGui::SliderFloat("Filled Opacity", &Instance<Settings>::Get()->Visuals.ESP.m_FilledOpacity, 0.0f, 1.0f, "%.1f");
								ImGui::Dummy({ 0, 10 });
								
								ImGui::SliderFloat("Outline Opacity", &Instance<Settings>::Get()->Visuals.ESP.m_OutlineOpacity, 0.0f, 1.0f, "%.1f");
								ImGui::Dummy({ 0, 10 });
								ImGui::SliderFloat("Outline Thickness", &Instance<Settings>::Get()->Visuals.ESP.m_OutlineThickness, 0.0f, 10.f, "%1.f");
								ImGui::Dummy({ 0, 10 });
						...
						case 4:
							...
							case 0:
								...
								if (ImGui::Button("\x30", { 210.f, 160.f })) {
									ShellExecuteA(NULL, "open", "https://discord.com/invite/vhSYmYQahr", NULL, NULL, SW_SHOWNORMAL);
								}
							...
							ImGui::BeginChild("##settingschild", { m_Width * 0.8f, m_Height * 0.8f }, true, ImGuiWindowFlags_NoScrollbar);
							{
								...
								if (ImGui::Button("Create", { ((m_Width * 0.8f) / 2.f) - 10.f, 30.f })) {
									if (strlen(configName) > 26 || strlen(configName) < 3)
										...
										MessageBoxA(NULL, "Disabled Currently", "", MB_ICONEXCLAMATION);
									}
								}
								ImGui::PopFont();
								...
								auto createConfig = [&](std::string name, int ID)
								{
									...
									ImGui::BeginChild(std::string("##config" + std::to_string(ID)).data(), { 160.f, 140.f }, false, ImGuiWindowFlags_NoScrollbar);
									{
										...
										if (ImGui::Button("Load")) {
											MessageBoxA(NULL, "Disabled Rn", "", MB_ICONEXCLAMATION);
										}
										ImGui::PopStyleColor(1);
